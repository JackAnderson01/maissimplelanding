...
export default async function DeclineBookingByCleanerController(req, res) {
  try {
    ...
    if (doesBookingExist == null) {
      ...
      if (
        doesBookingExist.serviceProvidersArray != null &&
        (doesBookingExist.serviceProvidersArray.length == 1 ||
          doesBookingExist.serviceProvidersArray.length == 0)
      ) {
...
const refundPayment = async (bookingId, result, res) => {
  if (
    result != null &&
    result.serviceProvidersArray.length > 0 &&
    result.progress == "pending"
  ) {
    return res
      .status(200)
      .send({ success: true, result: "Booking declined successfully" });
  }
