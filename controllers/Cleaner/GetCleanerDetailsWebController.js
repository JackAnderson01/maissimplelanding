...
export default async function GetCleanerDetailsWebController(req, res) {
  try {
    ...
    const cleaner = await ServiceProviderEntity.findOne({ email: email }).sort({
      _id: -1,
    });
    const bankDetails = await BankDetailsEntity.find({ email: email }).sort({
      _id: -1,
    });
    const walletDetails = await WalletEntity.findOne({ email: email }).sort({
      _id: -1,
    });
