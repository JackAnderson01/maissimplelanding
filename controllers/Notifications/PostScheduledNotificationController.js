...
export default async function PostScheduledNotificationController(req, res) {
  try {
    ...
    // Set status based on scheduled time and current time
    const status = scheduledTime > new Date() ? "scheduled" : "delivered";
    ...
    if (id != null) {
      ...
      if (existingScheduledNotification) {
        ...
        return res.status(404).send({
          success: false,
          error: "No such scheduled notification found",
        });
      }
    } else {
      // Create a new scheduled notification
    ...
    // Code block for sending notifications to everyone in the database
    if (audienceType == "all") {
